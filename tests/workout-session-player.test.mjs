...
test("attempt registration callback receives only a durably acknowledged start with its original UUID",async()=>{const userId=id(903);const started=begin({userId});await storage.mutateBundle(userId,b=>storage.saveChange(b,started,start));const original=globalThis.fetch;const acknowledged=[];try{globalThis.fetch=async(_url,opts)=>new Response(JSON.stringify({acknowledgements:JSON.parse(opts.body).events.map(e=>({id:e.id,status:"accepted"}))}));await storage.flushEvents(userId,async()=>userId,events=>acknowledged.push(...events.filter(e=>e.event_type==="workout_started")));assert.equal(acknowledged.length,1);assert.equal(acknowledged[0].attempt_id,started.checkpoint.attemptId);assert.equal(acknowledged[0].sequence,0);}finally{globalThis.fetch=original;}});
test("attempt RPC integration sends only identity parameters",()=>{const source=readFileSync(new URL("../src/components/workout/use-workout-session.ts",import.meta.url),"utf8");assert.match(source,/register_my_workout_session_attempt", \{ p_attempt_id: attemptId, p_session_id: input\.sessionId \}/);assert.match(source,/finalize_my_workout_session_attempt", \{ p_attempt_id: cp\.attemptId \}/);assert.doesNotMatch(source,/p_summary|started_at|elapsed_attempt_ms|measurement_quality/);});
