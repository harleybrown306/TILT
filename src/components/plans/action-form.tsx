...
export default function PlanActionForm({ action, children, label, confirmation, disabled = false }: {
  ...
  return <form action={formAction} onSubmit={(event) => {
    ...
    {state.message && <p role={state.status === "error" ? "alert" : "status"} className={state.status === "error" ? "text-rose-300" : "text-emerald-400"}>{state.message}</p>}
    <fieldset disabled={pending || disabled} className="space-y-4">
      {children}
      <button type="submit" disabled={pending || disabled} className="rounded-xl bg-emerald-500 px-5 py-3 font-semibold text-slate-950 hover:bg-emerald-400 disabled:cursor-not-allowed disabled:opacity-50">
