...
import { teamAnalytics, teamTrend } from "@/lib/team-analytics";
...
export default async function TeamAnalyticsPage({ params, searchParams }: Props) {
  ...
  const athleteRows = athletes.map((athlete) => ({ athlete, metrics: teamAnalytics(sessions.filter((session) => session.athleteUserId === athlete.id), today, days) }));
  ...
  return <main className="min-h-screen bg-slate-950 px-6 py-10 text-white"><div className="mx-auto max-w-6xl">
    ...
    <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5"><Metric label="Completed Workouts" value={summary.completedWorkouts}/><Metric label="Prescribed Minutes" value={displayMinutes(summary.prescribedWorkMs)}/><Metric label="Training Days" value={summary.trainingDays}/><Metric label="Attendance" value={summary.attendance === null ? "N/A" : `${summary.attendance}%`}/><Metric label="Active Athletes" value={summary.activeAthletes}/></section>
